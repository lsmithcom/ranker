<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-6">
      <h2 class="text-base font-semibold text-gray-900 mb-4">Add Keyword</h2>

      <form @submit.prevent="submit" class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Keyword <span class="text-red-500">*</span></label>
          <input
...
async function submit() {
  ...
  loading.value = true
  try {
    const data = await $fetch<{ success: boolean; data: unknown }>('/api/keywords', {
      method: 'POST',
      body: {
    ...
    error.value = e?.data?.message || 'Failed to add keyword'
  } finally {
    loading.value = false
  }
}
